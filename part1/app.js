    ...
    try {
        ...
        if (WalkRatingsRows[0].count === 0) {
            await db.execute(`
                ...
                VALUES
                ((SELECT request_id FROM WalkRequests WHERE duration_minutes = 10), (SELECT user_id FROM Users WHERE username = 'bobwalker'), (SELECT user_id FROM Users WHERE username = 'alice123'), 5, 'Really good walk or something'),
                ((SELECT request_id FROM WalkRequests WHERE duration_minutes = 40), (SELECT user_id FROM Users WHERE username = 'bobwalker'), (SELECT user_id FROM Users WHERE username = 'alice123'), 4, 'great walk');
                `);
        }
    } catch (err) {
...
app.get('/api/walkers/summary', async (req, res) => {
    try {
        const [walkersSummary] = await db.execute("SELECT u.username AS walker_username, COUNT(wra.rating_id) AS total_ratings, (ROUND(AVG(wra.rating), 1) AS FLOAT(3,1)) AS average_rating, COUNT(wr.request_id) AS completed_walks FROM Users u LEFT JOIN WalkRatings wra ON u.user_id = wra.walker_id LEFT JOIN WalkRequests wr ON wra.request_id = wr.request_id WHERE u.role = 'walker' GROUP BY u.user_id");
        res.json(walkersSummary);
    } catch (err) {
        res.status(500).json({ error: 'Failed to fetch summary of walkers', err });
    }
});
